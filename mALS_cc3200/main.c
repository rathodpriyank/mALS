...
MqttClientServer(void *pvParameters)
{
    ...
    UART_PRINT("Current Device mode : %d \r\n", g_deviceMode);
...
//			{
				if ( alarmCount == g_alarmStatus )
				{
					lRetVal =
					sl_ExtLib_MqttClientSend(BridgeHndl.ClientCtxHndl,
											 (const char *)pub_topics[0], "ALARM-ON",
											  strlen("ALARM-ON"), qos[0], 0 );
		...
		case ACCELERATION :
			{
				...
				lRetVal = sl_ExtLib_MqttClientSend(BridgeHndl.ClientCtxHndl,
											 (const char *)pub_topics[0], tempData,
											  strlen((char*)tempData), qos[0], 0 );
